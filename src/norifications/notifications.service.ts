...
export class NotificationsService {
  ...
  constructor(@InjectModel(Notification.name) private model: Model<NotificationDocument>  , @Inject(forwardRef(() => NotificationsGatway))  private readonly gatway:NotificationsGatway
    ) {
  ...
  async findAll(): Promise<Notification[]> {
    return this.model.find().limit(10).sort({'createAt':'asc'}).exec();
  }
